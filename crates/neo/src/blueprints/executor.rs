...
use blueprint_runtime::{NodeContext, NodeRegistry};
use blueprint_types::{
...
// Neo-specific types for downcasting from DynEvent/DynRequest
        ...
        match trigger {
            ...
            ExecutionTrigger::ServiceRequest { request_id, request } => {
                ...
                if let Some(req) = request {
                    // Downcast and serialize the request for the blueprint
                    if let Some(concrete_req) = req.downcast_ref::<ServiceRequest>() {
                        if let Ok(req_value) = serde_json::to_value(concrete_req) {
            ...
            ExecutionTrigger::ServiceEvent { event } => {
                if let Some(evt) = event {
                    // Downcast and serialize the event for the blueprint
                    if let Some(concrete_evt) = evt.downcast_ref::<Event>() {
                        if let Ok(evt_value) = serde_json::to_value(concrete_evt) {
...
mod tests {
    ...
    use blueprint_types::{Connection, PinType, VariableDef};
    ...
    async fn test_pure_function_execution() {
        use blueprint_types::{FunctionDef, FunctionParam, FUNCTION_ENTRY_NODE, FUNCTION_EXIT_NODE};
